...
import com.alibaba.rocketmq.common.utils.IOTinyUtils;
...
public class HttpTinyClient {
    ...
    public static String fetchNamesrvAddress(String url) throws IOException {
        HttpTinyClient.HttpResult result = HttpTinyClient.httpGet(url, null, null, "UTF-8", 15 * 1000);
        if (200 == result.code) {
            String responseStr = result.content;
            if (responseStr != null) {
                return clearNewLine(responseStr);
            } else {
                throw new IOException("fetch nameserver address is null");
            }
        } else {
            throw new IOException("fetch nameserver address failed. statusCode=" + result.code);
        }
    }
}
